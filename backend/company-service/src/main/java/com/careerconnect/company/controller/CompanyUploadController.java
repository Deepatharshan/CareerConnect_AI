...
@RestController
@RequestMapping("/api/v1/companies/uploads")
public class CompanyUploadController {
    ...
    public ResponseEntity<String> uploadLogo(@RequestParam("file") MultipartFile file) {
        ...
        try {
            String originalName = Optional.ofNullable(file.getOriginalFilename()).orElse("logo");
            String safeName = originalName.replaceAll("[^a-zA-Z0-9._-]", "_");
            String storedName = UUID.randomUUID() + "-" + safeName;
            
            Path uploadDir = Paths.get("uploads", "logos").toAbsolutePath();
            Files.createDirectories(uploadDir);
            
            Path filePath = uploadDir.resolve(storedName);
            Files.copy(file.getInputStream(), filePath);
            
            return ResponseEntity.ok("/api/v1/companies/uploads/logos/" + storedName);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> downloadLogo(@PathVariable String fileName) {
        try {
